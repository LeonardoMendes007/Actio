...
import controller.professor.EditarAtividadeController;
...
public class EditarAtividade {
	...
	private EditarAtividadeController controller;


	public EditarAtividade(BorderPane borderPrincipal, Atividade atividade) {
		this.borderPrincipal = borderPrincipal;
		this.atividade = atividade;

		
		controller = new EditarAtividadeController(this);
	...
	private void initTela() {
		...
		controller.addDiscComboBox(atividade.getDiscTurmaProf().getProfessor());

	//	controller.preencherCbDisciplina();
	}
	...
	private void initAddArquivo() {
		...
		file = new File(atividade.getPathArquivo());
	...
	private void initTurmaAtividade() {
		...
		controller.addTurmasComboBox(atividade.getDiscTurmaProf().getDisciplina());
	...
	private void initTextFieldDataEntrega() {
		...
		Date dataEntrega = new Date(atividade.getDtEntrega().getTime());
		
		dtEntrega = new DatePicker(dataEntrega.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
		dtEntrega.setPrefWidth(200);
		dtEntrega.setPrefHeight(30);
	//	dtEntrega.setValue(atividade.getDtEntrega().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
	...
	private void initTextFieldDataPublicacao() {
		Date dataPublicacao = new Date(atividade.getDtPublicacao().getTime());
		
		dtDataPublicacao = new DatePicker(dataPublicacao.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
		dtDataPublicacao.setPrefHeight(40);
		dtDataPublicacao.setPrefWidth(220);
		//dtDataPublicacao.setValue(atividade.getDtPublicacao().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
	...
	private void initTextField() {
		...
		tfAtividade.setText(atividade.getNome());
	...
	private void initLabelCriarAtividade() {
		...
		lblCriarAtividade = new Label("Editar Atividade");
